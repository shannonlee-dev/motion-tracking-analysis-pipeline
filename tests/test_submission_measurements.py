...
from scripts.evaluate_submission import associate, count_runs
from scripts.measure_submission import OUT


def rows(identities, excluded=()):
    return [dict(frame=i, track_id=identity, excluded=i in excluded)
...
def test_one_detection_cannot_identify_two_people():
    assigned = associate({'a': [0, 0, 20, 20], 'b': [0, 0, 20, 20]}, {'7': [0, 0, 20, 20]})
    assert list(assigned.values()).count('7') == 1


def test_recorded_feature_rows_equal_unmodified_app():
    recorded = list(csv.DictReader((OUT/'features.csv').open(encoding='utf-8-sig')))
    ...
    try:
        for index in range(max(selected)+1):
            ...
            if index in selected:
                ...
                assert float(row['rate']) == 100*result.matches/len(matcher.target_kp)
    finally:
        cap.release()


def test_every_summary_value_traces_to_event_rows():
    events = list(csv.DictReader((OUT/'event_results.csv').open(encoding='utf-8-sig')))
    summaries = list(csv.DictReader((OUT/'tracking_summary.csv').open(encoding='utf-8-sig')))
    specs = json.loads((OUT/'events.json').read_text())
    assert len(specs) == 30 and len({e['event_id'] for e in specs}) == 30
    assert sorted(int(r['trials']) for r in summaries if r['variant'] == 'default') == [5, 5, 10, 10]
    for summary in summaries:
        selected = [e for e in events if (e['variant'], e['condition']) == (summary['variant'], summary['condition'])]
        ...
        assert 100*sum(int(e['failed_event']) for e in selected)/len(selected) == float(summary['failure_rate'])
