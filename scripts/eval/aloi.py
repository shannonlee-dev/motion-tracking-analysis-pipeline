...
def main():
    ...
    variants = [(name, cv2.imread(str(root/f'1_{name}.png')), 0)
                for name in ('r0', 'r30', 'r60', 'l1c1', 'l8c1', 'i110', 'i250')]
    for fraction in (.3, .5):
    ...
    for name, im, covered in variants:
        ...
        scene = cv2.copyMakeBorder(im, 80, 80, 80, 80, cv2.BORDER_CONSTANT, value=(0, 0, 0))
        ...
        if desc is not None and len(desc) >= 2:
            pairs = bf.knnMatch(refdesc, desc, k=2)
            candidates = [p[0] for p in pairs if len(p) == 2 and p[0].distance < .75*p[1].distance]
            unique = {}
        ...
        if len(good) >= 4:
            ...
            _, inlier = cv2.findHomography(src, dst, cv2.RANSAC, 3)
        ...
        panel = cv2.resize(scene, (464, 368))
