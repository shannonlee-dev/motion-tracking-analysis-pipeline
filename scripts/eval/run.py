...
from scripts.constants import (
    EXPERIMENT_SEED, EXPERIMENT_THREADS, LEARNING_RATES, KERNEL_SIZES,
    DISTANCE_LIMITS, CURRENT_RESULTS_DIR,
)
...
def main():
    ...
    cv2.setNumThreads(EXPERIMENT_THREADS)
    ...
    variants={
        'baseline':config,
        **{f'lr_{rate}': replace(config, learning_rate=rate)
           for rate in LEARNING_RATES if rate != config.learning_rate},
        **{f'kernel_{size}': replace(config, kernel_size=size)
           for size in KERNEL_SIZES if size != config.kernel_size},
        **{f'distance_{distance}': replace(config, max_distance=distance)
    ...
    write_csv(RESULTS/CURRENT_RESULTS_DIR/'real_tracking.csv',evaluate_real(config, export_videos=args.export_videos))
    ...
    (RESULTS/'run.json').write_text(json.dumps(dict(python=platform.python_version(),opencv=cv2.__version__,
        numpy=np.__version__,config=asdict(config),seed=EXPERIMENT_SEED,threads=EXPERIMENT_THREADS,
