        ...
        for rate in (.001, .01, .1):
            ...
            try:
                for f, (image_path, gt_path) in enumerate(paths, 1):
                    ...
                    if any(tuple(c) not in ((0, 0, 255), (255, 255, 255)) for c in colored):
                    ...
                    if rate == .01:
                        ...
                        if f in (90, 120, 137, 150, 160, 170, 171, 180, 189, 190, 200, 230, 267, 275, 300, 330):
            ...
            if panels and name == 'I_CA_01':
                ...
                cv2.imwrite(str(results/f'captures/{name}_events.jpg'), np.concatenate(
                    [np.concatenate(panels[i:i+4], axis=1) for i in range(0, len(panels), 4)]))
